...
def train_model():
    ...
    for user in userlist:
        for imgname in os.listdir(f'static/faces/{user}'):
            ...
            labels.append(user)
            genders.append(user.split('_')[2])  # Extracting gender
            branches.append(user.split('_')[3])  # Extracting branch
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if cv2.waitKey(1) == ord('a'):
                add_attendance(identified_person, 'Unknown', 'Unknown')  # Pass 'Unknown' for gender and branch for now
...
def add():
    ...
    gender = request.form['gender']  # Extract gender
    branch = request.form['branch']  # Extract branch
    ...
    if totalreg() > 0:
        ...
        return redirect(url_for('home.html', names=names, rolls=rolls, genders=genders, branches=branches, times=times,
